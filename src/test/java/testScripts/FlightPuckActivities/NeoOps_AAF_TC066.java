...
public class NeoOps_AAF_TC066 {
	...
	void setUp() {
		...
		Driver.setUpTestExecution(tcName, "Air return & outbound data generation");
		// launch application
   		...
   		if(IFlightNeo_LoginPage.login(driver, username, password)) {
   			...
   			if(IFlightNeo_Gantt.changeGanttMode(driver, "Real World")) {
   			  ...
   			  if(IFlightNeo_Gantt.selectFlightInGantt(driver, actualOffTimeImg, "RIGHTCLICK")) {
   				...
   				com.performAction(driver, IFlightNeo_HomePage.menuOption_FlightReturnContinue(driver), "Click", "",
						"Clicking on flight return & continue");
   				...
   				com.performAction(driver, IFlightNeo_HomePage.groundReturn_time(driver),"SET", actgroundreturnTime, "Setting groundreturn time");
   				com.performAction(driver, IFlightNeo_HomePage.Revisedout_time(driver),"SET", revisedouttime, "Setting Revised Out time");
   				com.performAction(driver, IFlightNeo_HomePage.Revisedin_time(driver),"SET", revisedintime, "Setting Revised In time");
   				...
   				if(IFlightNeo_Gantt.selectFlightInGantt(driver, returnlegImg, "DoubleClick"))
						
					{
						...
						if (Fltstatus.equalsIgnoreCase("ENRT")&& Opsstatus.equalsIgnoreCase("FR"))
						{
						htmlLib.logReport("Open flight leg details Screen", "Flight leg details Open success & the Flight Return Flight Status is:"+Fltstatus+"ops status is:" +Opsstatus, "Pass", driver, true);	
						}
						
						else
							
						{
							htmlLib.logReport("Open flight leg details Screen", "Flight leg details Open success & the Flight Return Flight Status is not: "+Fltstatus+" ops status is not: " +Opsstatus, "Fail", driver, true);
						}
					}
				...
				// Close Window
			    	...
			    	if(IFlightNeo_Gantt.selectFlightInGantt(driver, newlegImg, "DoubleClick"))
			    	{
						...
						{
						htmlLib.logReport("Open flight leg details Screen", "Flight leg details Open success & the Flight Return Continuation Leg Flight Status is: "+Fltstatus+" ops status is: " +Opsstatus, "Pass", driver, true);	
						}
